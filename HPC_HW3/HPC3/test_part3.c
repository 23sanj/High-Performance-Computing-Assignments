...
int main (int argc, char *argv[])
{
   ...
   long long index= 1, primeindex = 1;		/* index of the current sieve	*/
   ...
   long long squareRoot= (long long ) sqrt((double) n) ;
   long long size1= (squareRoot)/2; 
    ...
    { prime=3; index=0; 
       do {
            prime = 3;  
                   ...
                   for (i1 = 0; i1 < Block_Size - first; i1 += prime)   marked[Block_Size * i +first + i1 ] = 1; 
                   index++; prime=prime1[index];
            } while (prime * prime < low_value + 2 * Block_Size);
       ...
       index = 0; prime = 3;
            ...
            do { 
                 ...
                 if (prime * prime > low_value) {first= (prime * prime - low_value)/2;}
                   ...
                   for (i1 = 0; i1 < Block_Size - first; i1 += prime) { if (Block_Size * i + first+ i1< size)  marked[Block_Size * i +first + i1 ] = 1; }
                   index++; prime=prime1[index];
