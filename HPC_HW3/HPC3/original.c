...
int main (int argc, char *argv[])
{
   ...
   unsigned long *marked;		/* array elements to be  marked	*/
   unsigned long  n;		/* value of the largest number	*/
   int p; 		/* number of processes		*/
   unsigned long  proc0_size;	/* number of elements assigned to process zero */
...
n = atoi(argv[1]);
   ...
   proc0_size = (n-1)/p;
   if ((2 + proc0_size) < (unsigned long ) sqrt((long double) n)) 
   {
      if (!id) 
   ...
   marked = (unsigned long *) malloc (size * sizeof(unsigned long));
   if (marked == NULL) 
   {
...
count = 0;
   ...
   elapsed_time += MPI_Wtime();
   if (!id) {
      printf ("%lf primes are less than or equal to %lf \n", global_count, n);
