...
if (argc != 2) 
{
   ...
   n = atoll(argv[1]);
	// converts long long integer values
   ...
   do {
      if (prime * prime > low_value)
       ...
       for (i = first; i < size; i += prime)      
        { marked[i] = 1; int prime1 = 2 * i + 3; }



       if (!id) {//id0 calculates the prime and broadcasts it to the other processes.
         while (marked[++index]);
         {prime = 2 * index + 3;
       
         }}
      
      MPI_Bcast (&prime,  1, MPI_INT, 0, MPI_COMM_WORLD);// the prime numbers calculated by process 1 are broadcasted to all other processes.
   } while (prime * prime <= n);

   count = 0;// count the total number of primes left unmarked by each process
   for (i = 0; i < size; i++)
      if (!marked[i]) {count++;
                       }
   MPI_Reduce (&count, &global_count, 1, MPI_INT, MPI_SUM,0, MPI_COMM_WORLD);// process0 collects the final count of the primes in the global count.
